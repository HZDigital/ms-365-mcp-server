...
function buildGeneralMcpInstructions(opts: McpInstructionsContext): string {
  const parts = [
    'Microsoft 365 MCP exposes Microsoft Graph and, when configured, Dynamics 365 CRM / Dataverse through MCP tools. Use each tool name, description, and parameter schema as the source of truth.',
    ...
    'Dynamics tools use one deployment-configured Dataverse organization. Use dynamics-get-current-user to retrieve the authenticated caller’s Dataverse UserId without searching by name. Use dynamics-list-tables and dynamics-get-table-metadata before querying custom tables. Generic Dynamics entity_set values are entity-set names, not URLs; activity writes use concrete task, phonecall, or appointment tools.',
  ];
