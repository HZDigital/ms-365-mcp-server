...
async function executeUtilityTool(
      ...
      content: [
        {
          ...
          text: JSON.stringify({
            ...
            destructive: true,
            message:
              'This tool modifies user data. Re-call with parameter "confirm": true after the user has explicitly approved this action.',
          }),
        },
      ],
      isError: true,
  ...
  try {
    ...
    const response = await utility.execute(toolParams, ctx);
    auditLog({
      event: 'tool.call',
...
export interface UtilityTool {
  ...
  stdioOnly?: boolean;
  /** Service-neutral manual tools can opt into organization mode without Graph scopes. */
  orgOnly?: boolean;
...
function installDeniedToolAuditHandler(
  ...
  if (!lowLevel) {
    logger.warn('Skipping denied-tool audit hook: MCP server internals are unavailable');
    return;
  }
  const handlers = (
...
export function registerGraphTools(
  ...
  accountNames: string[] = [],
  allowedScopesValue?: string,
  manualTools: readonly UtilityTool[] | boolean = [],
  httpMode: boolean = false
): number {
  const resolvedManualTools = Array.isArray(manualTools) ? manualTools : [];
  const resolvedHttpMode = typeof manualTools === 'boolean' ? manualTools : httpMode;
  ...
  for (const utility of [...UTILITY_TOOLS, ...resolvedManualTools]) {
    if (!orgMode && utility.orgOnly) continue;
    if (readOnly && !utility.readOnlyHint) continue;
    if (resolvedHttpMode && utility.stdioOnly) continue;
...
export function registerDiscoveryTools(
  ...
  const utilityTools = [...UTILITY_TOOLS, ...resolvedManualTools].filter((u) => {
    if (!orgMode && u.orgOnly) return false;
    if (readOnly && !u.readOnlyHint) return false;
    if (resolvedHttpMode && u.stdioOnly) return false;
